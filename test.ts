...
describe('ipfs-connector-utils', function () {
  ...
  it('adds an object to ipfs', function () {
    return helperInstance.add({ data: '{}' })
      .then((cid) => {
        ...
        expect(cid).to.have.property('codec');
        expect(cid).to.have.property('version');
      });
  });
  ...
  it('gets data from node', function() {
    return helperInstance.add({q: {v: 1}})
      .then((cid1) => helperInstance.get(cid1, '/q/v'))
      .then(result => {
  ...
  it('creates a cid with a linked property from data', function() {
    return helperInstance.addLinkFrom({firstName: 'John', lastName: 'Doe'}, 'name')
      .then(cid => {
        expect(cid).to.exist;
        expect(cid).to.have.property('multihash');
        expect(cid).to.have.property('codec');
        expect(cid).to.have.property('version');
        return helperInstance.get(cid, '/name/firstName');
      })
      .then(result => {
  ...
  it('creates a cid link from encoded string', function() {
    return helperInstance.addLink('zdpuB2CAUgUbyJ4s8SmqcGGaGfs7bG337dDNty8NSCR3fhgRw', 'details')
      .then(cid => {
        expect(cid).to.exist;
        expect(cid).to.have.property('multihash');
        expect(cid).to.have.property('codec');
        expect(cid).to.have.property('version');
        return helperInstance.get(cid, '/details/lastName');
      })
      .then(result => {
